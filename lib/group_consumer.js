...
GroupConsumer.prototype._fullRejoin = function () {
    ...
    return (function _tryFullRejoin(attempt) {
        ...
        if (attempt > 4) {
            throw new Error('Full rejoin retries exceeded');
        }
        ...
        self.generationId = 0;
        self.members = null;
        self.strategyName = null; // current strategy assigned by group coordinator
        ...
        .catch(errors.NoKafkaConnectionError, { code: 'UnknownMemberId' }, { code: 'NotCoordinatorForGroup' }, function (err) {
            ...
            return Promise.delay(1000).then(_tryFullRejoin(++attempt));
        });
    }())
    .tap(function () {
